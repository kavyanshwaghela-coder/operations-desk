...
// Self-Contained Layout Injector Engine
function loadWorkflowHtmlFiles() {
  ...
  // 1. FIXED RENDERING LIFECYCLE: Delayed injection to guarantee dropdown DOM availability
  setTimeout(function() {
    loadB2bCacheData();
...
// 5. ISOLATED WORKSPACE SWITCH ROUTER: Clears inputs and variables to prevent filter contamination
function activateModuleWorkflow(deptName, viewTarget) {
  ...
  // Clear persistent values upon section swaps to stop leakage between tabs
  ...
  var modTitle = document.getElementById('moduleTitle');
  if (modTitle) modTitle.innerText = workflowDept + " Workspace";
...
function renderTable(data) {
  ...
  if (!data || data.length === 0) { b.getElementById('rowCountLabel').innerText = "0"; b.innerHTML = `<tr><td colspan="11" class="text-center p-4 text-gray-400">No active records found matching criteria.</td></tr>`; return; }
...
// 3. WORKING LEDGER ARCHIVE EXPORTER: Added functional client-side filtered data CSV compiler engine
function exportFilteredDataToExcel() {
  ...
  currentlyFilteredData.forEach(function(r) {
    var line = `"${r.seriesNo}","${r.date} @ ${r.time}","${r.dept}","${r.transporter}","${r.vehicleNo}","${r.driverName}","${r.invoice}","${r.boxes}","${r.remark || r.lr}","${r.pallets || r.status}","${r.incharge}"`;
    csvContent += line + "\n";
  });
...
// 4. FIX MATCHING VARIABLES CASE: Populates past history meta box seamlessly
async function executeLiveDuplicateCheck(input) {
  ...
  if (res && res.isDuplicate) {
    ...
    // Exact mapping matches target document IDs
    ...
    var sBtn = document.getElementById('submitBtn'); if(sBtn) { sBtn.disabled = true; sBtn.innerText = "LOCKED"; }
  } else { 
...
// 2. FIXED PRODUCT MASTER CATALOUGE VIEW SEARCH ENGINE
...
function executeReprintQuery() {
  ...
  executeReprintAction(searchInput, reprintSource);
}

async function executeReprintAction(searchInput, reprintSource) {
  ...
  statusContainer.innerText = "Querying universal archives engine records...";
...
function saveToLocalStorage() {
  ...
  for(var i=0; i<rows.length; i++) {
    var n = rows[i].querySelector('.invoice-num')?.value;
    var b = rows[i].querySelector('.box-count')?.value;
    var l = rows[i].querySelector('.lr-no')?.value;
    var r = rows[i].querySelector('.row-remark')?.value || "";
  ...
  var cache = {
    ...
    transporter: document.getElementById('transporterSelect')?.value, vehicleNo: document.getElementById('vehicleNo')?.value,
    driverName: document.getElementById('driverName')?.value, driverMobile: document.getElementById('driverMobile')?.value,
    palletCount: document.getElementById('palletCount')?.value, incharge: document.getElementById('inchargeSelect')?.value,
    invoices: invoices
  };
  localStorage.setItem('draft_cache_b2b_dispatch', JSON.stringify(cache));
}

function loadB2bCacheData() {
  var raw = localStorage.getItem('draft_cache_b2b_dispatch'); if(!raw) return;
...
function saveInventoryToCache() {
  var rows = document.getElementById('inventoryItemsContainer')?.children || [];
  var items = [];
  for(var i=0; i<rows.length; i++) {
    items.push({
      item: rows[i].getAttribute('data-selected-item'), barcode: rows[i].getAttribute('data-selected-barcode'), sku: rows[i].getAttribute('data-selected-sku'),
      batch: rows[i].querySelector('.inv-batch-no')?.value, qty: rows[i].querySelector('.inv-total-qty')?.value, box: rows[i].querySelector('.inv-no-of-box')?.value, status: rows[i].querySelector('.inv-status')?.value
    });
  }
  var cache = {
    date: document.getElementById('invDateInput')?.value, poInvoiceNo: document.getElementById('invPoInvoiceNo')?.value, customerName: document.getElementById('invCustomerName')?.value,
    incharge: document.getElementById('invInchargeSelect')?.value, transporter: document.getElementById('invTransporterSelect')?.value, vehicleNo: document.getElementById('invVehicleNo')?.value,
    driverName: document.getElementById('invDriverName')?.value, driverNumber: document.getElementById('invDriverNumber')?.value, lrNo: document.getElementById('invLrNo')?.value,
    poTotalQty: document.getElementById('invPoTotalQty')?.value, poTotalBoxQty: document.getElementById('invPoTotalBoxQty')?.value, items: items
  };
  localStorage.setItem('draft_cache_inventory', JSON.stringify(cache));
}

function loadInventoryCacheData() {
  var raw = localStorage.getItem('draft_cache_inventory'); if(!raw) return;
  var cache = JSON.parse(raw);
...
function saveRtoToCache() {
  var awbInputs = document.querySelectorAll('.b2c-awb-input'); var awbArray = [];
  awbInputs.forEach(i => awbArray.push(i.value));
  var cache = {
    b2bDate: document.getElementById('rtoB2bDate')?.value, b2bInv: document.getElementById('rtoB2bInvoice')?.value, b2bLr: document.getElementById('rtoB2bLrNo')?.value, b2bTrans: document.getElementById('rtoB2bTransporterSelect')?.value, b2bBox: document.getElementById('rtoB2bBoxQty')?.value, b2bVeh: document.getElementById('rtoB2bVehicle')?.value, b2bDriver: document.getElementById('rtoB2bDriver')?.value, b2bMob: document.getElementById('rtoB2bMobile')?.value, b2bRemark: document.getElementById('rtoB2bRemark')?.value, b2bInch: document.getElementById('rtoB2bInchargeSelect')?.value,
    b2cDate: document.getElementById('rtoB2cDate')?.value, b2cVeh: document.getElementById('rtoB2cVehicle')?.value, b2cCourier: document.getElementById('rtoB2cCourierSelect')?.value, b2cInch: document.getElementById('rtoB2cInchargeSelect')?.value, b2cDriver: document.getElementById('rtoB2cDriver')?.value, b2cMob: document.getElementById('rtoB2cMobile')?.value, b2cRemark: document.getElementById('rtoB2cCommonRemark')?.value, b2cAwb: awbArray
  };
  localStorage.setItem('draft_cache_rto', JSON.stringify(cache));
}

function loadRtoCacheData() {
  var raw = localStorage.getItem('draft_cache_rto'); if(!raw) return;
  var cache = JSON.parse(raw);
  ...
  if(cache.b2cAwb && cache.b2cAwb.length > 0) {
    var c = document.getElementById('rtoB2cAwbContainer'); if(c) c.innerHTML = "";
    cache.b2cAwb.forEach(code => addRtoB2cAwbRow(code));
  }
}
...
function renderPrintLayout(d, id) {
  ...
  document.getElementById('pdfTime').innerText = d.time || "--:--"; 
  document.getElementById('pdfDept').innerText = d.department; 
  document.getElementById('pdfTransporter').innerText = d.transporter || "N/A"; 
  document.getElementById('pdfVehicle').innerText = d.vehicleNo || "N/A"; 
  document.getElementById('pdfDriver').innerText = d.driverName ? d.driverName + " (" + (d.driverMobile || d.driverNumber || "") + ")" : "N/A"; 
  document.getElementById('pdfIncharge').innerText = d.incharge || "Warehouse Team";
  
  var pdfTitle = document.getElementById('pdfTitle');
  var pdfSeriesLabel = document.getElementById('pdfSeriesLabel');
  var thEl = document.getElementById('pdfTableHeader');
  var bEl = document.getElementById('pdfTableBody'); bEl.innerHTML = "";
  ...
  if (d.department === "Inventory Inward" || d.department === "Inventory Logging") {
    ...
    if(invMetaCont) {
       ...
       document.getElementById('pdfInvPo').innerText = d.poInvoiceNo || "N/A";
       document.getElementById('pdfInvLr').innerText = d.lrNo || "N/A";
    }
